...
func execute(source_uuid: String, _targets: Array[String], battle_manager: Node, context: Dictionary) -> EffectResult:
	...
	if is_instance_valid(container):
		...
		print("[ESOD] Checking slot %d. Holder: %s, Slot Content: %s" % [holder_location.index, holder_uuid, slot_uuid])
		
		# Robust check: If slot is occupied AND it's not the holder (dead unit), find alternative
		if not slot_uuid.is_empty() and slot_uuid != holder_uuid:
			print("[ESOD] Slot occupied by %s. Finding alternative..." % slot_uuid)
			...
			print("[ESOD] Found empty slot: %d" % empty_slot)
			if empty_slot == -1:
