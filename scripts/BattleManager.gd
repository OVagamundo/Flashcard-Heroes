...
func _populate_slots_from_data(slots: Array, data: Array, container_name: StringName):
	for i in range(slots.size()):
		...
		if is_instance_valid(instance_data):
			...
			# Set orientation for enemy units
...
func _enter_combat_phase():
	...
	# Only change phase after combat is fully resolved
...
func _execute_combat_resolution():
	...
	var inventory_context = get_full_inventory_context()
	...
	for i in range(lineup_data.size()):
		var attacker = lineup_data[i]
		if not is_instance_valid(attacker) or attacker.current_hp <= 0:
			continue

		var target = _get_target_by_rule(true, &"Frontmost")
		if is_instance_valid(target):
			AbilityResolver.execute_effect(basic_attack_def.effect, attacker, [target], inventory_context)
			_check_for_deaths()
	...
	# --- CHECK FOR VICTORY ---
	if not _enemy_lineup_data.any(func(u): return is_instance_valid(u) and u.current_hp > 0):
		print("--- COMBAT PHASE END (VICTORY) ---")
		return

	# --- ENEMY TEAM'S TURN (Back-to-Front: Index 5 -> 0) ---
	print("Enemy team acts...")
	for i in range(_enemy_lineup_data.size() - 1, -1, -1):
		var attacker = _enemy_lineup_data[i]
		if not is_instance_valid(attacker) or attacker.current_hp <= 0:
			continue
...
func _get_opposing_lineup(is_player_unit: bool) -> Array[GachaBallInstance]:
	return _enemy_lineup_data if is_player_unit else lineup_data
