...
func bm_remove_instance(uuid: String) -> bool:
	...
	if loc.container == &"equipped_item":
		...
		# If this is a unit with equipped items, unequip and move them into player item inventory
	...
	if target_loc.container == &"equipped_item":
		...
		if is_instance_valid(b):
			...
			if source_loc.container == &"equipped_item":
				...
				b.location_container_tag = &"equipped_item"
		...
		a.location_container_tag = &"equipped_item"
		...
		if source_loc.container == &"equipped_item" and not is_instance_valid(b):
...
func bm_move_instance_to_discard(uuid: String) -> bool:
	...
	if index == -1:
		index = discard_container.get_all_uuids().size()
...
func bm_draw_gacha_instance(tier: int) -> bool:
	...
	if tier_pool.is_empty():
		var dest_container = get_container(container_tag)
		var to_move = get_instances_in_container(BATTLE_CONTAINER_TAGS.BATTLE_DISCARD_PILE).filter(
			func(inst): return inst.get_definition().tier == tier
		)
		if to_move.is_empty():
			return false
		SignalBus.emit_signal("battle_log_event", "Reshuffling Tier %d discard pile..." % tier)
		for inst in to_move:
			inst.reset_battle_stats()
			_remove_instance_from_container(inst)
			var insert_idx := dest_container.find_first_empty_slot()
			if insert_idx == -1:
				insert_idx = dest_container.get_all_uuids().size()
			dest_container.set_uuid(insert_idx, inst.ball_uuid)
			_update_instance_location(inst.ball_uuid, container_tag, insert_idx)
	...
	if get_instances_in_container(container_tag).is_empty():
		var dest2 = get_container(container_tag)
		var move_after = get_instances_in_container(BATTLE_CONTAINER_TAGS.BATTLE_DISCARD_PILE).filter(
			func(inst): return inst.get_definition().tier == tier
		)
		if not move_after.is_empty():
			SignalBus.emit_signal("battle_log_event", "Reshuffling Tier %d discard pile..." % tier)
			for inst2 in move_after:
				inst2.reset_battle_stats()
				_remove_instance_from_container(inst2)
				var ins := dest2.find_first_empty_slot()
				if ins == -1:
					ins = dest2.get_all_uuids().size()
				dest2.set_uuid(ins, inst2.ball_uuid)
				_update_instance_location(inst2.ball_uuid, container_tag, ins)
...
func _bm_validate_state_consistency() -> bool:
	...
	for cname in _containers.keys():
		...
		for i in range(uuids.size()):
			...
			if u.is_empty():
				continue
			if seen.has(u):
				push_error("Duplicate UUID %s found in battle containers" % u)
				return false
			seen[u] = true
			...
			if inst.get_location().container == &"equipped_item":
				push_error("Equipped item %s appears in a battle container slot" % u)
				return false
	for u in _battle_instances.keys():
		...
		if loc.container == &"equipped_item":
			...
			if not is_instance_valid(parent):
				print("[BM Validate] Equipped parent missing. item=", u, " parent=", loc.unit_uuid)
				# Auto-repair: clear linkage and rehome item to player inventory to prevent error spam
				inst.equipped_on_uuid = ""
				inst.equipped_slot_index = -1
				var discard_fix := get_container(BATTLE_CONTAINER_TAGS.BATTLE_DISCARD_PILE)
				if is_instance_valid(discard_fix):
					var d_idx_fix := discard_fix.find_first_empty_slot()
					if d_idx_fix == -1:
						d_idx_fix = discard_fix.get_all_uuids().size()
					discard_fix.set_uuid(d_idx_fix, u)
					_update_instance_location(u, BATTLE_CONTAINER_TAGS.BATTLE_DISCARD_PILE, d_idx_fix)
				print("[BM Validate] Auto-repaired orphan equipped item ", u)
				continue
			if loc.index < 0 or loc.index >= parent.equipped_item_uuids.size():
				# Auto-repair bad index by clearing equipped state and rehoming
				inst.equipped_on_uuid = ""
				inst.equipped_slot_index = -1
				var discard_fix2 := get_container(BATTLE_CONTAINER_TAGS.BATTLE_DISCARD_PILE)
				if is_instance_valid(discard_fix2):
					var d_idx_fix2 := discard_fix2.find_first_empty_slot()
					if d_idx_fix2 == -1:
						d_idx_fix2 = discard_fix2.get_all_uuids().size()
					discard_fix2.set_uuid(d_idx_fix2, u)
					_update_instance_location(u, BATTLE_CONTAINER_TAGS.BATTLE_DISCARD_PILE, d_idx_fix2)
				print("[BM Validate] Auto-repaired equipped index for ", u)
				continue
			if parent.equipped_item_uuids[loc.index] != u:
				# Auto-repair mapping mismatch by clearing item linkage and rehoming
				inst.equipped_on_uuid = ""
				inst.equipped_slot_index = -1
				var discard_fix3 := get_container(BATTLE_CONTAINER_TAGS.BATTLE_DISCARD_PILE)
				if is_instance_valid(discard_fix3):
					var d_idx_fix3 := discard_fix3.find_first_empty_slot()
					if d_idx_fix3 == -1:
						d_idx_fix3 = discard_fix3.get_all_uuids().size()
					discard_fix3.set_uuid(d_idx_fix3, u)
					_update_instance_location(u, BATTLE_CONTAINER_TAGS.BATTLE_DISCARD_PILE, d_idx_fix3)
				print("[BM Validate] Auto-repaired equipped mapping for ", u)
				continue
		else:
			...
			if actual != u:
				# Detailed diagnostics to identify the source of mismatch
				var snapshot := c2.get_all_uuids()
				print("[BM Validate] Mismatch for ", u,
					" loc=(", String(loc.container), ":", loc.index, ")",
					" actual_slot=", actual,
					" container_size=", snapshot.size(),
					" snapshot=", snapshot)
				push_error("Index/truth mismatch (battle) for %s" % u)
				return false
	return true
...
func _remove_instance_from_container(instance: GachaBallInstance):
	...
	# Clear stale location to avoid index/truth mismatches until the instance is re-placed or erased
	_update_instance_location(instance.ball_uuid, &"", -1)
