# res://scripts/engine/actions/TransformBlackMarketAction.gd
...
func _init(p_target_uuid: String = "", p_cost: int = 5) -> void:
	super._init(&"TransformBlackMarketAction")
...
func execute() -> void:
	...
	if is_instance_valid(bm) and not ActionQueue.is_headless_mode() and bm.has_method("execute_transform_visuals"):
		bm.execute_transform_visuals(target_uuid, cost)
	else:
		# Direct data mutation
		var instance = GameManager.run_state.get_instance_by_uuid(target_uuid)
		if is_instance_valid(instance):
			var source_definition = instance.get_definition()
			var source_loc = GameManager.run_state.get_location_for_uuid(target_uuid)
			var result_def = GameManager.get_transform_result(source_definition)
			if is_instance_valid(result_def) and is_instance_valid(source_loc):
