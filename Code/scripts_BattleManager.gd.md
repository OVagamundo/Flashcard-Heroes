...
func _exit_tree():
	...
	if EventBus.is_connected("inventory_action_requested", _on_inventory_action_requested):
		EventBus.inventory_action_requested.disconnect(_on_inventory_action_requested)
	if EventBus.is_connected("choice_made", _on_choice_made):
		EventBus.choice_made.disconnect(_on_choice_made)
...
func _setup_battle():
	...
	if is_instance_valid(hero_run_instance):
		...
		printerr("CRITICAL: BattleManager could not find Hero instance!")
...
func _connect_signals():
	...
	discard_pile_button.pressed.connect(_on_display_discard_pile_requested)
...
func _on_inventory_action_requested(source_view: Control, target_view: Control):
	...
	elif source_def.category == target_def.category:
...
func _handle_swap(source_view: GachaBallView, target_view: GachaBallView):
	...
	var source_idx = source_view.get_index()
	var target_idx = target_view.get_index()
	
	source_parent.remove_child(source_view)
	target_parent.remove_child(target_view)
	
	source_parent.add_child(target_view)
	source_parent.move_child(target_view, source_idx)
	target_parent.add_child(source_view)
	target_parent.move_child(source_view, target_idx)
...
func _on_display_discard_pile_requested():
	WindowManager.open_workspace_window(&"DiscardPile", {"discard_pile": self._discard_pile})
