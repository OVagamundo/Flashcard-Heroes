...
W6	The Selection Persistence Rule	When opening an inspection window for a GachaBall, the parent entity remains selected. Selection only changes if the player explicitly clicks on an equipped item or another entity, which will then transfer selection to that new entity.	Maintains visual context for the player—the selection highlight indicates which entity is being inspected. Without this, the player loses the visual anchor when navigating to child windows.
