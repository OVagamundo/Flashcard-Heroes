...
## 1.1 Snapshot Architecture: The "Rewind" Mechanism
...
## 1.6 Universal Sub-Phase Payload Architecture

Complex animations (like multi-hit attacks, chaining attacks, or intercepting blows) require proper sequencing without breaking the Zero-Time Simulation rule. The Universal Sub-Phase Payload architecture manages this:

1. **Self-Contained Effect Scripts**: Complex abilities (like `BasicAttackEffect`) handle their own execution and sub-phase logic internally, rather than relying on a monolithic `EffectHandlers` switch statement. They explicitly calculate interactions (like Burn application, Fire Traits, and Guardian intercepts) before computing final damage impacts.
2. **Sub-Phase Arrays**: `CombatEvent`s can contain nested arrays of sub-events (e.g., an `events` array within the `visual_payload`). This allows the `BattleAnimator` to unpack and play complex sequences (like "Lunge" -> "Hit" -> "Return") in order, dynamically handling sub-routines.
3. **Type-Safe Resource Nesting**: `CombatEvent` extends `Resource` (which supports deep copying natively without stripping object metadata) to prevent Godot's `duplicate(true)` from mangling inner arrays of objects into untyped dictionaries. Arrays of events MUST be strongly typed as `Array[Resource]` before being nested in a dictionary payload.

## 1.7 Damage & Attack Types Matrix
