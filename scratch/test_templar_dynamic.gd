...
const VisualDataAdapter = preload("res://scripts/VisualDataAdapter.gd")
const AbilityResolver = preload("res://scripts/AbilityResolver.gd")

func _init() -> void:
	print("--- Starting Templar Dynamic Ability Verification Test ---")
...
func _run_tests() -> void:
	...
	events = bm._combat.process_reaction_queue(bm, {})
	# Both player_templar and trained_templar should have received events
	# player_templar: base 3 + 4 tokens = 7 PWR
	# trained_templar: base 5 (trained) + 4 tokens = 9 PWR
	...
	events = bm._combat.process_reaction_queue(bm, {})
	# Enemy templar should scale with player's 6 tokens: 3 + 6 = 9 PWR
